...
use crate::utils::{self, SerializableResultItem};
...
pub fn receive_arp_responses(
    ...
    loop {
        ...
        if let Some(arp) = arp_packet {
            ...
            let serializable_hostname = match &target_details.hostname {
                Some(hostname) => hostname.clone(),
                None => String::from(""),
            };

            let serializable_vendor = match &target_details.vendor {
                Some(vendor) => vendor.clone(),
                None => String::from(""),
            };

            let serializable_details = SerializableResultItem {
                ipv4: format!("{}", (&target_details).ipv4),
                mac: format!("{}", (&target_details).mac),
                hostname: serializable_hostname,
                vendor: serializable_vendor,
            };

            match options.output {
                OutputFormat::Plain => {
                    println!("{} ({}) -> {} from {}", sender_ipv4, "-", sender_mac, "-");
                }
                OutputFormat::Json => {
                    println!("{}", serde_json::to_string(&serializable_details).unwrap());
                }
    ...
    let target_details = discover_map
        ...
        .map(|mut target_detail| {
            ...
            let serializable_hostname = match &target_detail.hostname {
                Some(hostname) => hostname.clone(),
                None => String::from(""),
            };

            let serializable_vendor = match &target_detail.vendor {
                Some(vendor) => vendor.clone(),
                None => String::from(""),
            };

            let serializable_details = SerializableResultItem {
                ipv4: format!("{}", (&target_detail).ipv4),
                mac: format!("{}", (&target_detail).mac),
                hostname: serializable_hostname,
                vendor: serializable_vendor,
            };
            ...
            match options.output {
                OutputFormat::Plain => {
                    println!(
                        "{} ({}) -> {} from {}",
                        &target_detail.ipv4,
                        &target_detail.hostname.clone().unwrap_or("-".to_string()),
                        &target_detail.mac,
                        &target_detail.vendor.clone().unwrap_or("-".to_string())
                    );
                }
                OutputFormat::Json => {
                    println!("{}", serde_json::to_string(&serializable_details).unwrap());
